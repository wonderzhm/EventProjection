#' @name EventProjection-package
#' @aliases EventProjection-package EventProjection
#'
#' @title Event Projection
#'
#' @description Predicts enrollment and event outcomes at the design or
#' interim-analysis stage using parametric enrollment, dropout, and
#' time-to-event models with simulation-based projection.
#'
#' @details The \code{EventProjection} package originated as a fork of
#' the \code{eventPred} package and is maintained and extended by
#' Haiming Zhou. Relative to \code{eventPred}, it incorporates cure rate
#' and delayed treatment effect functionality adapted from
#' \code{EventPredInCure} and further adds support for
#' user-specified fixed cure rates within the event projection workflow.
#'
...
#' monitoring, and execution of clinical trials. The \code{EventProjection}
#' package provides enrollment, dropout, and event prediction capabilities
#' at both the design stage and the interim-analysis stage. At the design
#' stage, users may project future enrollment and event outcomes from
#' assumed model parameters and trial operating characteristics. At the
#' interim-analysis stage, users may fit models to blinded or unblinded
#' ongoing trial data and simulate future enrollment, dropout, and event
#' trajectories conditional on the observed data.
#'
...
#' before enrollment completion, the \code{EventProjection} package
...
#'
#' The \code{EventProjection} package also offers several time-to-event
#' models, including exponential, Weibull, log-logistic, log-normal,
#' piecewise exponential, model averaging of Weibull and log-normal,
#' spline, and Cox models. In addition, the package supports cure rate
#' and delayed treatment effect models, with cure rate models fitted
#' either by estimating the cure fraction or by using a user-specified
#' fixed cure rate. For time to dropout, the standard parametric,
#' spline, and piecewise models described above are available where
#' applicable. If enrollment is complete, ongoing subjects who have
#' not had the event
...
#'
#' The \code{EventProjection} package displays the Akaike Information
...
#'
#' The most useful function in the \code{EventProjection} package is
...
#' prediction model.
#'
#' The \code{EventProjection} package implements a model
...
#' }
#'
#' The \code{EventProjection} package uses days as its primary time unit.
#' If you need to convert enrollment or event rates per month to
#' rates per day, simply divide by 30.4375.
#'
#' @author Haiming Zhou (maintainer and extensions);
#'   Kaifeng Lu (author of the original \code{eventPred} package)
#'
#' @references
