...
#' @importFrom EventPredInCure predictEvent predictEnrollment summarizeObserved
#' @importFrom EventPredInCure fitEnrollment fitDropout
#'
#' @examples
#' \donttest{
...
#'                    vtheta=0)
#'
#' getPrediction2(target_n=200,target_d=60,lags=46,enroll_prior=fit1,
#'               event_prior=fit2,
  ...
  if (grepl("event", to_predict, ignore.case = TRUE)) {
    if (!is.null(df)) { # event prediction at analysis stage
      ...
      if ((!by_treatment && observed$d0 > 0) ||
        ...
        event_fit <- fitEvent2(df = df, event_model, cure_rate,
