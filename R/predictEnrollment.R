...
#'
#' @note
#' This function is based on the original implementation in the \pkg{eventPred}
#' package by Kaifeng Lu. The piecewise uniform enrollment option and an optional
#' The optional \code{seed.num} argument was adapted from \pkg{EventPredInCure}, and the merged
#' version standardizes inputs/outputs to support a unified code base.
#'
#' @references
...
predictEnrollment <- function(df = NULL, target_n = NA,
  ...
  if (!is.null(seed.num)) {
    if (length(seed.num) != 1 || is.na(seed.num) || !is.numeric(seed.num)) {
      stop("seed.num must be a single non-missing numeric/integer value.")
    }
  ...
  if ((model == "poisson" && p != 1) ||
      ...
      (model %in% c("piecewise poisson", "piecewise uniform") &&
  ...
  if (model %in% c("piecewise poisson", "piecewise uniform")) {
  ...
  if (tolower(enroll_fit$model) == "poisson") {
    ...
    newEnrollment_bs <- function(t0, n1, theta, x, lags, nreps) {
      df = data.table::setDT(list(draw = numeric(nreps*n1)))
       ...
       window_start = max(1L, t0x - lags)
       lambdaT = colMeans(lambda[window_start:t0x, , drop = FALSE])
    ...
    # Piecewise uniform enrollment model (deterministic within-interval spacing
    # conditional on the (possibly drawn) enrollment rates).
    #
    # This option was adopted from EventPredInCure (see package-level ACK below).
    # It parallels the piecewise Poisson model, but uses unit increments on the
    # cumulative mean scale instead of exponential increments.
    ...
    newEnrollment_pwu <- function(t0, n1, theta, u, nreps) {
      ...
      for (i in 1:nreps) {
        ...
        # unit increments correspond to evenly spaced arrivals on the
        # cumulative mean scale (a "piecewise uniform" mechanism)
    ...
    # UPDATED (merge): model-specific simulation ends here; shared post-processing below
  }
