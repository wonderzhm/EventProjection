...
#' @param seed.num Optional integer seed for reproducible simulation. If
#'   provided (non-\code{NULL}), \code{set.seed(seed.num)} is called at the
#'   start of the function. If \code{NULL} (default), no seed is set.
#' @param return_new_events Logical; if \code{TRUE} (default), keep the
#'   subject-level simulated event data in the returned object. Setting this
#'   to \code{FALSE} can reduce memory use for large \code{nreps}.
#'
#' @details
#' This merged implementation is based on the original \code{predictEvent()} from the
#' \pkg{eventPred} package by Kaifeng Lu, with additional features adapted from
#' \pkg{EventPredInCure}:
#' \itemize{
#'   \item Added support for Chen (2016) style mixture-cure event models, including optional
#'   delayed-treatment-effect variants.
#'   \item Added optional \code{seed.num} for reproducible simulation (only used when provided).
#'   \item Made parameter draws robust to singular covariance matrices (e.g., when the cure fraction
#'   is fixed in \code{fitEvent()} and the corresponding variance is 0).
#'   \item Hardened the quantile/root-finding step for estimating the time to reach \code{target_d}
#'   to avoid NA/-Inf failures when few/no events are observed in simulated datasets.
#' }
#'
...
#'
#'
#' Tai-Tsang Chen. Predicting analysis times in randomized clinical trials with cancer immunotherapy.
...
predictEvent <- function(df = NULL, target_d = NA, newSubjects = NULL,
                         ...
                         fix_parameter = FALSE,
                         generate_plot = TRUE,
                         interactive_plot = TRUE, seed.num = NULL,
                         return_new_events = TRUE) {
  ...
  erify::check_bool(return_new_events)
  
  # Optional reproducibility control (adapted from EventPredInCure)
  if (!is.null(seed.num)) {
    if (!is.numeric(seed.num) || length(seed.num) != 1L || is.na(seed.num)) {
      stop("seed.num must be a single non-missing numeric value.")
    }
  ...
  if (!is.null(event_fit)) {
    ...
    for (j in 1:ngroups) {
      ...
      if (!is.null(covariates_event) && grepl("with cured population", model)) {
        stop("Cure models with covariates_event are not currently supported in predictEvent().")
      }
      ...
      if ((model == "exponential" && p != 1) ||
          ...
          (model == "piecewise exponential with cured population" &&
           !(p %in% c(length(event_fit[[j]]$piecewiseSurvivalTime),
                      length(event_fit[[j]]$piecewiseSurvivalTime) + 1))) ||
      ...
      if (grepl("^piecewise exponential", model)) {
  ...
  if (!by_treatment) {
    if (!is.null(newSubjects)) {
      dfb1 <- .ep_summarize_count_matrix(
        .ep_count_matrix_from_values(nt, "arrivalTime", t, nreps),
        t,
        pilevel
      )
  ...
  # Safe multivariate normal sampler that tolerates singular covariance matrices.
  # (Needed for fixed-cure-rate fits where the cure parameter has variance 0.)
  # UPDATED (merge): robust MVN draws when 'sigma' is singular/near-singular (e.g., fixed cure fraction => Var=0)
  rmvnorm_safe <- function(n, mean, sigma, eps = 1e-12) {
    mean <- as.numeric(mean)
    p <- length(mean)
    if (is.null(sigma)) {
      return(matrix(mean, nrow = n, ncol = p, byrow = TRUE))
    }
    sigma <- as.matrix(sigma)
    if (!all(dim(sigma) == c(p, p))) {
      stop("sigma must be a square matrix with dimensions length(mean) x length(mean).")
    }
    sigma <- (sigma + t(sigma)) / 2
    v <- diag(sigma)
    keep <- which(is.finite(v) & v > eps)
    
    out <- matrix(mean, nrow = n, ncol = p, byrow = TRUE)
    if (length(keep) == 0) return(out)
    
    sigma_k <- sigma[keep, keep, drop = FALSE]
    sigma_k <- (sigma_k + t(sigma_k)) / 2
    ee <- eigen(sigma_k, symmetric = TRUE)
    vals <- pmax(ee$values, 0)
    L <- ee$vectors %*% diag(sqrt(vals), nrow = length(vals))
    
    z <- matrix(rnorm(n * length(keep)), nrow = n)
    draws_k <- z %*% t(L)
    out[, keep] <- draws_k + matrix(mean[keep], nrow = n, ncol = length(keep), byrow = TRUE)
    out
  }
  
  # Use mvtnorm::rmvnorm when possible to preserve eventPred/EventPredInCure RNG behavior;
  # fall back to rmvnorm_safe() only when sigma is singular/invalid (e.g., fixed cure fraction).
  rmvnorm_ep <- function(n, mean, sigma, eps = 1e-12) {
    if (is.null(sigma)) return(rmvnorm_safe(n, mean = mean, sigma = sigma, eps = eps))
    out <- tryCatch(
      mvtnorm::rmvnorm(n, mean = as.numeric(mean), sigma = as.matrix(sigma)),
      error = function(e) NULL
    )
    if (is.null(out)) out <- rmvnorm_safe(n, mean = mean, sigma = sigma, eps = eps)
    out
  }
  ...
  for (i in 1:nreps) {
    ...
    if (n1 > 0) {
      ...
      for (j in 1:ngroups) {
        ...
        if (ncols > 0) {
          ...
          if (model == "exponential with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            rate <- exp(theta[2])
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "exponential", p = pc, time0 = 0, a = 1, b = 1 / rate) + 1
            }
            ...
            b <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "weibull", p = pc, time0 = 0, a = a, b = b) + 1
            }
            ...
            b <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "log-logistic", p = pc, time0 = 0, a = a, b = b) + 1
            }
            ...
            sdc <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "log-normal", p = pc, time0 = 0, mu = muc, sd = sdc) + 1
            }
          } else if (model == "piecewise exponential with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            lambda <- exp(theta[-1])
            u <- event_fit[[j]]$piecewiseSurvivalTime
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time_piecewise_exp_abovetime0(uj[tindex], p = pc, time0 = 0, piecewiseSurvivalTime = u, piecewisehazard = lambda) + 1
            }
            ...
            hrj <- theta[4]
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time(uj[tindex], hr = hrj, distribution = "exponential", p = pc, lag = lag, a = 1, b = 1 / rate) + 1
            }
            ...
            hrj <- theta[5]
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time(uj[tindex], hr = hrj, distribution = "weibull", p = pc, lag = lag, a = a, b = b) + 1
            }
            ...
            hrj <- theta[5]
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time(uj[tindex], hr = hrj, distribution = "log-logistic", p = pc, lag = lag, a = a, b = b) + 1
            }
            ...
            hrj <- theta[5]
            for (tindex in seq_len(ncols)) {
              survivalTimeNew[cols[tindex]] <- Chen_2016_event_time(uj[tindex], hr = hrj, distribution = "log-normal", p = pc, lag = lag, mu = muc, sd = sdc) + 1
            }
          ...
          } else if (grepl("^piecewise exponential", model)) {
            J = length(theta)
      ...
      for (j in 1:ngroups) {
        ...
        if (ncols > 0) {
          ...
          if (model == "exponential with cured population") {
            ...
            rate <- exp(theta[2])
            for (tindex in seq_len(ncols)) {
              survivalTimeOngoing[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "exponential", p = pc, time0 = u0[tindex], a = 1, b = 1 / rate)
            }
          } else if (model == "weibull with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            ...
            b <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeOngoing[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "weibull", p = pc, time0 = u0[tindex], a = a, b = b)
            }
          } else if (model == "log-logistic with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            ...
            b <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeOngoing[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "log-logistic", p = pc, time0 = u0[tindex], a = a, b = b)
            }
          } else if (model == "log-normal with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            ...
            sdc <- exp(theta[3])
            for (tindex in seq_len(ncols)) {
              survivalTimeOngoing[cols[tindex]] <- Chen_2016_event_time_abovetime0(uj[tindex], distribution = "log-normal", p = pc, time0 = u0[tindex], mu = muc, sd = sdc)
            }
          } else if (model == "piecewise exponential with cured population") {
            uj <- stats::runif(ncols)
            pc <- exp(theta[1]) / (1 + exp(theta[1]))
            ...
            u <- event_fit[[j]]$piecewiseSurvivalTime
            for (tindex in seq_len(ncols)) {
              survivalTimeOngoing[cols[tindex]] <- Chen_2016_event_time_piecewise_exp_abovetime0(uj[tindex], p = pc, time0 = u0[tindex], piecewiseSurvivalTime = u, piecewisehazard = lambda)
            }
          } else if (model == "exponential") {
            rate = exp(theta)
    ...
    if (!fixedFollowup) {
      if (!is.null(dropout_fit) || !is.null(dropout_fit_w_x)) {
        ...
        eventx = 1*(timex == survivalTime & timex<10^10)
  ...
  sdf <- function(t, target_d, d0, newEvents) {
    # UPDATED (merge): use na.rm=TRUE to avoid NA propagating into if() checks
    sumdata <- newEvents[, list(
      n = sum(get("totalTime") <= t & get("event"), na.rm = TRUE)), by = "draw"]
    mean(sumdata$n < target_d - d0)
  }
  
  tmax_event = newEvents[get("event") == 1, max(get("totalTime"), na.rm = TRUE)]
  tmax_all = newEvents[, max(get("totalTime"), na.rm = TRUE)]
  tmax = if (is.finite(tmax_event)) tmax_event else tmax_all
  ...
  target_times <- NULL
  if (!all(is.na(target_t))) {
    target_times <- target_t + t0
    t = sort(unique(c(t, target_times)))
  }

  overall_event_count_mat <- NULL
  if (!by_treatment || !is.null(target_times)) {
    overall_event_count_mat <- .ep_count_matrix_from_values(
      newEvents[get("event") == 1],
      "totalTime",
      t,
      nreps
    )
  ...
  if (!is.null(target_times)) {
    pred_at_t <- .ep_summarize_target_times(
      count_mat = overall_event_count_mat,
      times = t,
  ...
  if (!by_treatment) {
    event_count_mat <- overall_event_count_mat
    dropout_count_mat <- .ep_count_matrix_from_values(
      newEvents[get("dropout") == 1],
      "totalTime",
      t,
      nreps
    )
    arrival_count_mat <- .ep_count_matrix_from_values(
      newEvents,
      "arrivalTime",
      t,
      nreps
    )
    completed_count_mat <- .ep_count_matrix_from_values(
      newEvents,
      "totalTime",
      t,
      nreps
    )
    ongoing_count_mat <- arrival_count_mat - completed_count_mat
    
    dfb2 <- .ep_summarize_count_matrix(event_count_mat, t, pilevel)
    dfb2 <- .ep_add_summary_offset(dfb2, d0)
    
    dfb3 <- .ep_summarize_count_matrix(dropout_count_mat, t, pilevel)
    dfb3 <- .ep_add_summary_offset(dfb3, c0)
    
    dfb4 <- .ep_summarize_count_matrix(ongoing_count_mat, t, pilevel)
    ...
    if (!is.null(df)) {
      ...
      dfa4 <- data.table::rbindlist(list(
        df0,
        .ep_observed_ongoing_summary(
          times = t2,
          arrival_values = dt$arrivalTime,
          completed_values = dt[get("event") | get("dropout")]$totalTime,
          pilevel = pilevel
        )), use.names = TRUE)[
                         get("t") <= tp][order(get("t"))]
      ...
      # add time tp
      dfa2 <- .ep_append_summary_timepoint(dfa2, tp)
      dfa3 <- .ep_append_summary_timepoint(dfa3, tp)
      dfa4 <- .ep_append_summary_timepoint(dfa4, tp)
    ...
    dfb_list <- lapply(seq_len(nrow(sum_by_trt)), function(i) {
      trt_i <- sum_by_trt$treatment[i]
      desc_i <- sum_by_trt$treatment_description[i]
      sim_dt <- if (trt_i == 9999) {
        newEvents
      } else {
        newEvents[get("treatment") == trt_i]
      }
      
      event_count_mat <- .ep_count_matrix_from_values(
        sim_dt[get("event") == 1],
        "totalTime",
        t,
        nreps
      ...
      event_dt <- .ep_summarize_count_matrix(event_count_mat, t, pilevel)
      event_dt <- .ep_add_summary_offset(event_dt, sum_by_trt$d0[i])
      event_dt[, `:=`(treatment = trt_i, treatment_description = desc_i)]
      
      dropout_dt <- .ep_summarize_count_matrix(dropout_count_mat, t, pilevel)
      dropout_dt <- .ep_add_summary_offset(dropout_dt, sum_by_trt$c0[i])
      dropout_dt[, `:=`(treatment = trt_i, treatment_description = desc_i)]
      
      ongoing_dt <- .ep_summarize_count_matrix(ongoing_count_mat, t, pilevel)
      ongoing_dt[, `:=`(treatment = trt_i, treatment_description = desc_i)]
      
      list(event = event_dt, dropout = dropout_dt, ongoing = ongoing_dt)
    })
    
    dfb2 <- data.table::rbindlist(lapply(dfb_list, `[[`, "event"), use.names = TRUE)
    dfb3 <- data.table::rbindlist(lapply(dfb_list, `[[`, "dropout"), use.names = TRUE)
    dfb4 <- data.table::rbindlist(lapply(dfb_list, `[[`, "ongoing"), use.names = TRUE)
    ...
    if (!is.null(df)) {
      ...
      dfa4 <- data.table::rbindlist(list(
        df0,
        .ep_observed_grouped_ongoing_summary(
          groups_dt = sum_by_trt[, mget(trtcols)],
          data_dt = df2,
          group_cols = trtcols,
          times = t2,
          pilevel = pilevel
      ...
      # add time tp
      dfa2 <- .ep_append_summary_timepoint(dfa2, tp, group_cols = trtcols)
      dfa3 <- .ep_append_summary_timepoint(dfa3, tp, group_cols = trtcols)
      dfa4 <- .ep_append_summary_timepoint(dfa4, tp, group_cols = trtcols)
  ...
  if (return_new_events) {
    out$newEvents <- newEvents
  }
  
  out$enroll_pred_df <- enroll_pred_df
  out$event_pred_df <- event_pred_df
  out$dropout_pred_df <- dropout_pred_df
  out$ongoing_pred_df <- ongoing_pred_df
  
  if (generate_plot && (showEnrollment || showEvent ||
                        showDropout || showOngoing)) {
    out$event_pred_plot <- g1
  }
