...
f_exponential_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Exponential' || input.event_prior == 'Exponential with cured population' || input.event_prior == 'Exponential with cured population and delayed treatment') && input.k ==", i),
...
f_weibull_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Weibull' || input.event_prior == 'Weibull with cured population' || input.event_prior == 'Weibull with cured population and delayed treatment') && input.k ==", i),
...
f_lnorm_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Log-normal' || input.event_prior == 'Log-normal with cured population' || input.event_prior == 'Log-normal with cured population and delayed treatment') && input.k ==", i),
...
f_piecewise_exponential_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Piecewise exponential' || input.event_prior == 'Piecewise exponential with cured population') && input.k ==", i),
...
f_cure_rate_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Exponential with cured population' || input.event_prior == 'Weibull with cured population' || input.event_prior == 'Log-logistic with cured population' || input.event_prior == 'Log-normal with cured population' || input.event_prior == 'Piecewise exponential with cured population' || input.event_prior == 'Exponential with cured population and delayed treatment' || input.event_prior == 'Weibull with cured population and delayed treatment' || input.event_prior == 'Log-logistic with cured population and delayed treatment' || input.event_prior == 'Log-normal with cured population and delayed treatment') && input.k ==", i),

    shinyMatrix::matrixInput(
      paste0("cure_rate_survival_", i),
      label = "Cure rate for each treatment",
      value = matrix(rep(0.20, i), nrow = 1,
                     dimnames = list(NULL, paste("Treatment", 1:i))),
      inputClass = "numeric",
      rows = list(names = FALSE, extend = FALSE),
      cols = list(names = TRUE, extend = FALSE)
...
f_delay_effect_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Exponential with cured population and delayed treatment' || input.event_prior == 'Weibull with cured population and delayed treatment' || input.event_prior == 'Log-logistic with cured population and delayed treatment' || input.event_prior == 'Log-normal with cured population and delayed treatment') && input.k ==", i),

    shinyMatrix::matrixInput(
      paste0("delay_effect_survival_", i),
      label = "Delayed-treatment effect parameters",
      value = matrix(rep(c(46, 0.70), i), nrow = 2, byrow = FALSE,
                     dimnames = list(c("Lag (days)", "Hazard ratio after lag"),
                                     paste("Treatment", 1:i))),
      inputClass = "numeric",
      rows = list(names = TRUE, extend = FALSE),
      cols = list(names = TRUE, extend = FALSE)
...
f_cure_llogis_survival <- function(i) {
  conditionalPanel(
    condition = paste("(input.event_prior == 'Log-logistic with cured population' || input.event_prior == 'Log-logistic with cured population and delayed treatment') && input.k ==", i),

    shinyMatrix::matrixInput(
      paste0("cure_llogis_survival_", i),
      label = "Log-logistic parameters",
      value = matrix(rep(c(1.2, 400), i), nrow = 2, byrow = FALSE,
                     dimnames = list(c("Shape", "Scale"),
                                     paste("Treatment", 1:i))),
      inputClass = "numeric",
      rows = list(names = TRUE, extend = FALSE),
  ...
  conditionalPanel(
    ...
    fluidRow(
      ...
      column(6,
             ...
             conditionalPanel(
               condition = "input.enroll_prior == 'Piecewise Poisson' || input.enroll_prior == 'Piecewise Uniform'",
...
eventPanel <- tabPanel(
  ...
  conditionalPanel(
    ...
    fluidRow(
      ...
      column(8,
             ...
             lapply(1:6, f_cure_llogis_survival),
             lapply(1:6, f_lnorm_survival),
             lapply(1:6, f_piecewise_exponential_survival),
             lapply(1:6, f_cure_rate_survival),
             lapply(1:6, f_delay_effect_survival)
      )
    )
  ),
  ...
  conditionalPanel(
    ...
    fluidRow(
      ...
      column(6,
             conditionalPanel(
               condition = "input.event_model == 'Exponential with cured population' || input.event_model == 'Weibull with cured population' || input.event_model == 'Log-logistic with cured population' || input.event_model == 'Log-normal with cured population' || input.event_model == 'Piecewise exponential with cured population'",

               checkboxInput(
                 "use_fixed_cure_rate",
                 label = "Fix cure rate instead of estimating it?",
                 value = FALSE
               ),

               conditionalPanel(
                 condition = "input.use_fixed_cure_rate",

                 textInput(
                   "fixed_cure_rate_text",
                   label = "Fixed cure rate(s) in treatment order (one value overall, or comma-separated by treatment)",
                   value = "0.20"
                 )
               )
             ),
             ...
             conditionalPanel(
               condition = "input.event_model == 'Piecewise exponential' || input.event_model == 'Piecewise exponential with cured population'",
  ...
  titlePanel(tagList(
    span(HTML(paste(tags$span(style="font-size:14pt",
         ...
         span(actionButton(
           ...
           fileInputNoExtra("loadInputs", label=NULL, accept=c(".RData", ".rda", ".rdata"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        fileInput(
          "file1",
          label = "Upload subject level data (.xlsx, .csv, .rda/.RData)",
          accept = c(".xlsx", ".csv", ".rda", ".RData", ".rdata")
        )
      ),
      ...
      conditionalPanel(
        condition = "input.to_predict == 'Enrollment and event' || input.stage == 'Real-time after enrollment completion'",

        checkboxInput(
          "fixedFollowup",
          label = "Use fixed follow-up design?",
          value = FALSE
        ),

        conditionalPanel(
          condition = "input.fixedFollowup",

          numericInput(
            "followupTime",
            label = "Fixed follow-up time (days)",
            value = 365,
...
server <- function(input, output, session) {
  ...
  followupTime <- reactive({
    ...
    shinyFeedback::feedbackWarning(
      "followupTime", !valid,
      "Fixed follow-up time must be a positive number")
    req(valid)
    as.numeric(input$followupTime)
  })


  fixed_cure_rate <- reactive({
    if (!isTRUE(input$use_fixed_cure_rate)) return(NULL)
    txt <- trimws(input$fixed_cure_rate_text)
    if (!nzchar(txt)) return(NULL)
    vals <- as.numeric(trimws(unlist(strsplit(txt, ","))))
    valid <- length(vals) > 0 && all(is.finite(vals)) && all(vals > 0 & vals < 1)
    if (!valid) {
      showNotification("Fixed cure rate(s) must be numeric values strictly between 0 and 1.")
    }
    req(valid)
    vals
  })
  ...
  observeEvent(treatment_description(), {
    if (input$stage == "Design stage") {
      ...
      updateMatrixInput(
        session, paste0("cure_rate_survival_", k()),
        value=matrix(cure_rate_survival(), nrow=1,
                     dimnames = list(NULL, treatment_description())))

      updateMatrixInput(
        session, paste0("delay_effect_survival_", k()),
        value=matrix(delay_effect_survival(), nrow=2, ncol=k(),
                     dimnames = list(c("Lag (days)", "Hazard ratio after lag"),
                                     treatment_description())))

      updateMatrixInput(
        session, paste0("cure_llogis_survival_", k()),
        value=matrix(cure_llogis_survival(), nrow=2, ncol=k(),
                     dimnames = list(c("Shape", "Scale"),
                                     treatment_description())))
  ...
  cure_rate_survival <- reactive({
    req(k())
    param = input[[paste0("cure_rate_survival_", k())]]
    rate = as.numeric(param)
    valid = all(rate > 0 & rate < 1)
    if (!valid) {
      showNotification("Cure rate must be strictly between 0 and 1")
    }
    req(valid)
    rate
  })
  ...
  cure_llogis_survival <- reactive({
    req(k())
    param = input[[paste0("cure_llogis_survival_", k())]]
    shape = as.numeric(param[1,])
    scale = as.numeric(param[2,])

    valid1 = all(shape > 0)
    if (!valid1) {
      showNotification("Log-logistic shape parameter must be positive")
    }

    valid2 = all(scale > 0)
    if (!valid2) {
      showNotification("Log-logistic scale parameter must be positive")
    }
  ...
  delay_effect_survival <- reactive({
    req(k())
    param = input[[paste0("delay_effect_survival_", k())]]
    lag = as.numeric(param[1,])
    hr = as.numeric(param[2,])

    valid1 = all(lag >= 0)
    if (!valid1) {
      showNotification("Lag time must be nonnegative")
    }

    valid2 = all(hr > 0)
    if (!valid2) {
      showNotification("Hazard ratio after lag must be positive")
    }

    req(valid1 && valid2)

    matrix(c(lag, hr), nrow = 2, byrow = TRUE)
  })
  ...
  df <- reactive({
    ...
    df <- switch(
      ext,
      xlsx = data.table::setDT(readxl::read_excel(inFile$datapath)),
      csv = data.table::setDT(data.table::fread(inFile$datapath)),
      rda = {
        e <- new.env(parent = emptyenv())
        obj_names <- load(inFile$datapath, envir = e)
        objs <- mget(obj_names, envir = e)
        keep <- vapply(objs, function(x) inherits(x, "data.frame"), logical(1))
        shiny::validate(need(any(keep),
          "The uploaded .rda file must contain at least one data.frame-like object."))
        data.table::setDT(data.table::copy(objs[[which(keep)[1]]]))
      },
      rdata = {
        e <- new.env(parent = emptyenv())
        obj_names <- load(inFile$datapath, envir = e)
        ...
        keep <- vapply(objs, function(x) inherits(x, "data.frame"), logical(1))
        shiny::validate(need(any(keep),
          "The uploaded .RData file must contain at least one data.frame-like object."))
        data.table::setDT(data.table::copy(objs[[which(keep)[1]]]))
      },
      stop("Unsupported file type. Please upload .xlsx, .csv, .rda, or .RData.")
    )
    ...
    if (any(is.na(df[, ..req_cols]))) {
      stop(paste("The following columns have missing values:",
                 paste(req_cols[sapply(df[, ..req_cols], function(x) any(is.na(x)))],
                       collapse = ", ")))
    }
    ...
    for (nm in intersect(c("trialsdt", "randdt", "cutoffdt"), names(df))) {
      df[[nm]] <- as.Date(df[[nm]])
    }
  ...
  pred <- eventReactive(input$predict, {
    ...
    if (input$stage == "Design stage") {
      ...
      } else if (input$enroll_prior %in% c("Piecewise Poisson", "Piecewise Uniform")) {
      ...
      if (input$enroll_prior %in% c("Piecewise Poisson", "Piecewise Uniform")) {
      ...
      if (to_predict() == "Enrollment and event") {
        ...
        for (i in 1:k()) {
          if (model == "Exponential") {
            ...
            theta = c(qlogis(cure_rate_survival()[i]), log(exponential_survival()[i]))
          } else if (model == "Weibull with cured population") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      log(weibull_survival()[1,i]),
                      log(weibull_survival()[2,i]))
          } else if (model == "Log-logistic with cured population") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      log(cure_llogis_survival()[1,i]),
                      log(cure_llogis_survival()[2,i]))
          } else if (model == "Log-normal with cured population") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      lnorm_survival()[1,i],
                      log(lnorm_survival()[2,i]))
          } else if (model == "Piecewise exponential with cured population") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      log(piecewise_exponential_survival()[,i+1]))
            piecewiseSurvivalTime = piecewise_exponential_survival()[,1]
          } else if (model == "Exponential with cured population and delayed treatment") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      log(exponential_survival()[i]),
                      delay_effect_survival()[1,i],
                      delay_effect_survival()[2,i])
          } else if (model == "Weibull with cured population and delayed treatment") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      log(weibull_survival()[1,i]),
                      log(weibull_survival()[2,i]),
                      delay_effect_survival()[1,i],
            ...
            theta = c(qlogis(cure_rate_survival()[i]),
                      ...
                      delay_effect_survival()[1,i],
                      delay_effect_survival()[2,i])
          } else if (model == "Log-normal with cured population and delayed treatment") {
            theta = c(qlogis(cure_rate_survival()[i]),
                      lnorm_survival()[1,i],
                      log(lnorm_survival()[2,i]),
                      delay_effect_survival()[1,i],
      ...
      if (to_predict() == "Enrollment only") {
        ...
        getPrediction(
          ...
          fixedFollowup = isTRUE(input$fixedFollowup),
  ...
  output$event_pred_at_t <- renderText({
    if ((to_predict() == "Enrollment and event" ||
      ...
      if (!is.null(pred()$event_pred$pred_at_t)) {
        dx <- pred()$event_pred$pred_at_t[1, ]
        if (input$stage != "Design stage") {
        ...
        str3 <- paste0("Prediction interval: ", round(dx$lower),
                       ", ", round(dx$upper))
        str4 <- if ("prob_gt_target_d" %in% names(dx)) {
          paste0("Predictive probability of reaching ", dx$target_d,
                 " events by then: ", round(100 * dx$prob_gt_target_d, 1), "%")
        } else {
          NULL
        }
        text3 <- paste(c(paste("<b>", str1, "</b>"), str2, str3, str4),
                       collapse = "<br/>")
      } else {
        text3 <- NULL
      }
  ...
  output$saveInputs <- downloadHandler(
    filename = function() {
      paste0("inputs_", Sys.Date(), "_EventProjection.RData")
    },
    ...
    content = function(file) {
      x <- list(
        ...
        target_n = if (!is.null(input$target_n) && nzchar(as.character(input$target_n))) input$target_n else NA,
        target_d = if (!is.null(input$target_d) && nzchar(as.character(input$target_d))) input$target_d else NA,
        pilevel = pilevel(),
        nyears = nyears(),
        pred_at_t = input$pred_at_t,
        target_t = if (isTRUE(input$pred_at_t) && !is.null(input$target_t)) input$target_t else NA,
        fixedFollowup = isTRUE(input$fixedFollowup),
        followupTime = if (!is.null(input$followupTime)) input$followupTime else 365,
        use_fixed_cure_rate = isTRUE(input$use_fixed_cure_rate),
        fixed_cure_rate_text = input$fixed_cure_rate_text,
        ...
        piecewise_exponential_survival = matrix(
          ...
          dimnames = list(paste("Interval",
                          ...
                          c("Starting time", treatment_description()))),
        cure_rate_survival = matrix(
          cure_rate_survival(), nrow = 1,
          dimnames = list(NULL, treatment_description())),
        cure_llogis_survival = matrix(
          cure_llogis_survival(), nrow = 2,
          dimnames = list(c("Shape", "Scale"), treatment_description())),
        delay_effect_survival = matrix(
  ...
  observeEvent(input$loadInputs, {
    ...
    valid <- (tolower(ext) %in% c("rdata", "rda"))
    if (!valid) showNotification("Please upload an .rda or .RData file")
    req(valid)
    ...
    updateRadioButtons(session, "pilevel", selected=as.character(x$pilevel))
    updateNumericInput(session, "nyears", value=x$nyears)
    if (!is.null(x$fixedFollowup)) {
      updateCheckboxInput(session, "fixedFollowup", value = x$fixedFollowup)
    }
    if (!is.null(x$followupTime)) {
      updateNumericInput(session, "followupTime", value = x$followupTime)
    }
    if (!is.null(x$use_fixed_cure_rate)) {
      updateCheckboxInput(session, "use_fixed_cure_rate", value = x$use_fixed_cure_rate)
    }
    if (!is.null(x$fixed_cure_rate_text)) {
      updateTextInput(session, "fixed_cure_rate_text", value = x$fixed_cure_rate_text)
    }
    ...
    if (x$stage == "Design stage") {
      ...
      } else if (x$enroll_prior %in% c("Piecewise Poisson", "Piecewise Uniform")) {
        updateMatrixInput(
    ...
    if (x$stage == "Design stage") {
      ...
      if (!is.null(x$cure_llogis_survival) && grepl("Log-logistic with cured population", x$event_prior, fixed = TRUE)) {
        updateMatrixInput(
          session, paste0("cure_llogis_survival_", x$k),
          value=x$cure_llogis_survival)
      }

      if (!is.null(x$cure_rate_survival) && grepl("with cured population", x$event_prior, fixed = TRUE)) {
        updateMatrixInput(
          session, paste0("cure_rate_survival_", x$k),
          value=x$cure_rate_survival)
      }

      if (!is.null(x$delay_effect_survival) && grepl("delayed treatment", x$event_prior, fixed = TRUE)) {
        updateMatrixInput(
          session, paste0("delay_effect_survival_", x$k),
          value=x$delay_effect_survival)
      }

      if (grepl("^Log-normal", x$event_prior)) {
        updateMatrixInput(
          session, paste0("lnorm_survival_", x$k),
          value=x$lnorm_survival)
      }

      if (grepl("^Piecewise exponential", x$event_prior)) {
        updateMatrixInput(
      ...
      if ((x$stage == "Real-time before enrollment completion" &&
        ...
        if (x$event_model %in% c("Piecewise exponential", "Piecewise exponential with cured population")) {
          updateMatrixInput(
